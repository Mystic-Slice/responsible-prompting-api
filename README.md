...
You can also add a reference for the sentence in the `references` array and use it inside the `ref` key. Note that both the `embedding` and `centroid` keys will be populated by a model after obtaining the embeddings at step 2.
 ...
 Once the input file has been edited, either to change, include sentences or to create more labels, the embeddings need to be generated by the model and the centroids to be calculated.
 
 1. After editing `prompt_sentences.json` file, go back into our root folder and run `customize/customize_embeddings.py`
```
cd.. responsible-prompting-api/
